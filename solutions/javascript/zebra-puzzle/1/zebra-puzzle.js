...
class Solver {
  ...
  #same(houses, index, key1, value1, key2, value2) {
    ...
    const length1 = houses[index][key1].length;
    const length2 = houses[index][key2].length;

    if (this.#isSolution(houses, index, key1, value1)) {
      for (let i = 0; i < length2; i++) {
        const currentValue = houses[index][key2][i];
        if (currentValue !== value2) {
          this.#remove(houses, index, key2, currentValue);
        }
      }
    }
    ...
    if (this.#isSolution(houses, index, key2, value2)) {
      for (let i = 0; i < length1; i++) {
        const currentValue = houses[index][key1][i];
        if (currentValue !== value1) {
          this.#remove(houses, index, key1, currentValue);
        }
      }
    }
  }

const scoreFullHouse = (rolls) => {
  ...
  const last_sorted  = sorted[4];

  if (first_sorted !== last_sorted
  &&  sorted[1] === sorted[0]
  &&  sorted[3] === sorted[4]
...
export const score = (rolls, category) => {
  switch (category) {
    case 'ones':
      if (!rolls.some(r => r === 1)) return 0;
      return rolls.filter(r => r === 1).reduce((sum, roll) => sum += roll);
    case 'twos':
      if (!rolls.some(r => r === 2)) return 0;
      return rolls.filter(r => r === 2).reduce((sum, roll) => sum += roll);
    case 'threes':
      if (!rolls.some(r => r === 3)) return 0;
      return rolls.filter(r => r === 3).reduce((sum, roll) => sum += roll);
    case 'fours':
      if (!rolls.some(r => r === 4)) return 0;
      return rolls.filter(r => r === 4).reduce((sum, roll) => sum += roll);
    case 'fives':
      if (!rolls.some(r => r === 5)) return 0;
      return rolls.filter(r => r === 5).reduce((sum, roll) => sum += roll);
    case 'sixes':
      if (!rolls.some(r => r === 6)) return 0;
      return rolls.filter(r => r === 6).reduce((sum, roll) => sum += roll);
    case 'full house':

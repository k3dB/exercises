class Game
  ...
  def roll(pins)
    ...
    raise BowlingError.new "Invalid roll." unless valid?(pins)
  end

  def score
    raise BowlingError.new "Cannot score an incomplete game." unless complete_game?
  ...
  def apply_extras_to_prior_frames(num, pins)
    ...
    @frames[@current_frame - num].apply_extra(pins)
  end

class TwoBucket
  ...
  def initialize(bucket_one_size, bucket_two_size, goal, starting_bucket)
    @moves        = 0
    @goal_bucket  = ""
    @other_bucket = 0 # Amount of non-goal bucket

    non_starting_bucket = BUCKET_TWO if starting_bucket == BUCKET_ONE
    non_starting_bucket = BUCKET_ONE if starting_bucket == BUCKET_TWO

    if starting_bucket == BUCKET_ONE
      first_bucket  = Bucket.new(bucket_one_size)
      second_bucket = Bucket.new(bucket_two_size)
    else
      first_bucket  = Bucket.new(bucket_two_size)
      second_bucket = Bucket.new(bucket_one_size)
    end

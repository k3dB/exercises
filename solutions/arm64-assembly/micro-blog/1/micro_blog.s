...
truncate:
    ...
    mov   x6, #1           // Byte counter
.next:
    ldrb  w3, [x1], #1     // Load one byte at a time
    strb  w3, [x0], #1
    sub   x6, x6, #1       // Decrement byte counter
    ...
    beq   .truncate
    mov   x6, #1           // Default to one byte count
    b     .next

.two_bytes:
    mov   x6, #1           // One more byte
    b     .next

.three_bytes:
    mov   x6, #2           // Two more bytes
    b     .next

.four_bytes:
    mov   x6, #3           // Three more bytes
    b     .next

.truncate:
    mov   w3, wzr          // Null-terminate truncated value
    strb  w3, [x0]
.done:
    ret

...
get_box_weight:
    ...
    movzx eax, cx                  ; Get weight of second product, first,
    mul   edx                      ; because RDX gets clobbered by MUL
    mov   rcx, rax                 ; Preserve weight of first product
    movzx eax, di                  ; Get weight of first product
    mul   esi                      ; 32-bit multiplication to get full result in RAX
    add   eax, ecx                 ; Add both product weights
    add   eax, WEIGHT_OF_EMPTY_BOX ; Add weight of box for total weight
    ret
...
calculate_payment:
    ...
    inc   r9b              ; Add self to total payment pool
    movzx r9, r9b          ; Zero extend 8-bit value
    mov   rax, r10
    cqo

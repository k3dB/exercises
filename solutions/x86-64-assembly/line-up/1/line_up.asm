...
.next_digit:
    ...
    xor   r8, r8              ; Flag set if digit in tens place is a one
    xor   r9, r9              ; Flog set if there is only one digit
    mov   r11, 1              ; Bit to conditionally set the flags with
    cmp   ecx, 1              ; Check if there is only one digit
    cmove r9, r11             ; If so, set flag
...
.unwind_digits:
    ...
    cmove r8, r11             ; If so, set flag
.skip_tens_check:
    dec   ecx                 ; Rewind count of digits
    test  ecx, ecx            ; Any more digits?
    jnz   .unwind_digits

    test  r8, r8              ; If a one is not in the tens place,
    je    .compare_special    ; check for special case

    cmp   r9, 1               ; If there is only one digit,
    je    .compare_special    ; check for special case

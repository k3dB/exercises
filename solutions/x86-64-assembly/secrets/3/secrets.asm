...
extract_redundant_bits:
    ; This function takes one 16-bit integer as argument.
    ; It returns a 8-bit integer with all bits set in both the lower and the
    ; higher 8 bits of the argument.
    mov ax, di
    shr di, 8
    and al, dil
    ret
...
set_message_bits:
    ; This function takes one 16-bit integer as argument.
    ; It returns a 8-bit integer with all bits set if they are set in the higher
    ; 8 bits of the argument, the others unchanged.
    mov ax, di
    shr di, 8
    or  al, dil
    ret
...
rotate_private_key:
    ; This function takes one 16-bit integer as argument.
    ; It returns a 16-bit integer with bits of the private key rotated to the
    ; left a number of positions equal to the redundant bits. A bit is
    ; redundant when it is set in both the lowest 8-bit portion of the argument
    ; and the highest 8-bit portion of the argument.
...
format_private_key:
    ...
    mov    ax, PRIVATE_KEY
    rol    ax, cl
    ; - Isolate the lowest 8-bit portion of the rotated private key,
    ; which is the base value.
    mov    dx, ax
    ; - Isolate the highest 8-bit portion of the rotated private key,
    ; which is a mask to be
...
decrypt_message:
    ; This function takes one 16-bit integer as argument.
    ; The SI register is used to preserve values instead of using the
    ; stack since it is known that SI is not used in the other functions.
    mov  si, di
    ; It returns a 16-bit integer, of which:
    ; - The higher 8 bits are the formatted private key,
    ;   according to 'format_private_key'
